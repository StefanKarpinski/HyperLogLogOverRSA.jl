...
1. **Anonymity is preserved exactly.** The encryption and the per-request re-randomization reveal *no more* about a client than a plaintext HyperLogLog value would. Layer 3 cannot make privacy any worse than layers 1 and 2 already are. (Proven in [Proof of Anonymity](05-security-analysis.md).)
2. **The server can’t smuggle in a fingerprint.** A maliciously constructed RSA modulus *could* secretly tag clients, so the server must publish a certificate proving its modulus is “fingerprint-free.” Clients verify this certificate before using a ring. (Proven in [Malicious servers](05-security-analysis.md#Malicious-servers).)
...
If you want to dig into the details: HyperLogLog and resource class sharding are covered in [Anonymously Counting Users](03-counting-users.md), and the two proofs are in [Proof of Anonymity](05-security-analysis.md) (anonymity) and [Malicious servers](05-security-analysis.md#Malicious-servers) (fingerprint-freedom).
