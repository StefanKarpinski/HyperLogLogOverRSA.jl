...
What *is* safe to expose is **aggregate** output for large enough slices. Per-slice cardinality can be published, rather than the per-request $(b, k)$ values themselves. This is safe so long as a **minimum-cardinality floor** is enforced: refuse to report any slice whose estimated cardinality is below a threshold $T$, folding everything beneath it into a single “other” bucket.

The floor has a concrete rationale rather than an arbitrary cutoff. A published estimate hides an individual precisely when it cannot betray whether that individual is present—and we can measure how close it comes. Adding or removing one client shifts the *expected* estimate by exactly $1$, while the estimate itself carries a standard error of about $1.04\,n/\sqrt{B}$. Their ratio, $\sqrt{B}/(1.04\,n)$, is the detectability of a single client: when it is well below $1$, the estimator’s own sampling noise drowns out any one client’s contribution. So $\sqrt{B}$ ($\approx 64$ for $B = 2^{12}$) is too low—there a lone client is fully visible—but only a small multiple is needed: a floor of four to eight times $\sqrt{B}$, a few hundred clients, keeps single-client detectability between roughly a quarter and an eighth, with the round number $1024$ at the conservative end. This bounds a *single* published estimate; an adversary who differences or repeatedly queries overlapping slices averages the noise away and must be limited separately.
