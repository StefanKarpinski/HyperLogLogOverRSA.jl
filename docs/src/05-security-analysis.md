...
The exponent $B2^{m-1}$ is even ($m ≥ 3$), so every $z^{B2^{m-1}}$ has positive Jacobi symbol. Since $N ≡ 5 \bmod 8$ we have $-1 \in J_N^+$, so the entire noise group has positive Jacobi symbol. This subgroup is where we sample “noise” values to randomize the parts of $x$ that don’t encode the HyperLogLog value. We previously described deriving individual $w$ values from random $z$ values; here we consider the entire group.
...
The anonymity theorem is a *within-class* statement: in any single resource class, two clients with the same sketch are indistinguishable. Real clients, though, do not touch one class at a time. A single Pkg operation—resolving an environment, installing a project’s dependencies—fans out into a *bundle* of requests spanning many classes at once, over one connection, from one address, in one burst. The server can group such a bundle as coming from a single client with near-certainty, without breaking any cryptography. The honest question is therefore not only what one class leaks, but what a bundle of classes leaks *jointly*.

This is what settles a design choice deferred from [the construction](04-hyperloglog-over-rsa.md#What-we-actually-do). Each client’s value must be independent across classes; the choice is whether *both* coordinates of the sketch shard per class—an independent bucket *and* rank in every class (*full sharding*)—or only the rank, with the bucket held fixed across the client’s classes (*semisharding*).

Consider a bundle covering $j$ classes; the server decodes a $(b_i, k_i)$ pair for each. Under **full sharding** the bucket indices $b_1, \dots, b_j$ are independent and uniform on $\Z_B$, and—being deterministic in the master key and class—*stable*: the same client running the same operation next week reproduces the very same bucket vector. That vector is a fingerprint of $j \log_2 B$ bits (for $B = 2^{12}$, twelve bits per class): with even a handful of classes it is unique across the whole client population, and because it is stable and server-decodable it functions as a persistent tracking identifier. Full sharding *multiplies* the bucket leak by the number of classes in a bundle, turning a coarse coordinate into a per-client tag.

Under **semisharding** the bucket is the client’s single master-key bucket $b$, the same in every class, so a bundle exposes *one* $\log_2 B$-bit value no matter how many classes it spans—shared by roughly $1/B$ of all clients, far too coarse to identify anyone. The only per-class variation left is the rank $k_i$, which is exactly the sketch content the server legitimately needs and is mostly small, common values. The rank still shards—a client rare in one class is independently, and almost surely commonly, ranked in the next—so the [per-class rarity argument](03-counting-users.md#Resource-class-sharding) is untouched. Counting is unaffected: HLL recovers a cardinality from a per-bucket maximum within each class, and a client sitting in one fixed bucket contributes correctly to every class’s sketch.

So fixing the bucket costs no anonymity—a lone bucket index singles out no one—while sharding it would hand the server a stable, high-entropy fingerprint assembled from exactly the bundles it can already observe. That asymmetry is the whole reason the [shard base](04-hyperloglog-over-rsa.md#What-we-actually-do) $f$ annihilates the bucket coordinate: we shard the rank and fix the bucket.
...
If a malicious client sends $y$ with $\Jacobi_N(y) ≠ -1$, the server will detect it. So we can focus on a malicious client sending $y$ with $\Jacobi_N(y) = -1$. As discussed in the previous section, the values it can build from a fixed twist $x_0$ (with $\Jacobi_N(x_0) = -1$) and a semigenerator $g$ are those of the form $y = \pm x_0 g^h$; take $y = x_0 g^h$, the other case being essentially the same. Here we use $h$ as just an arbitrary attacker-controlled exponent value, not necessarily the output of a hash function. The question is whether a malicious client can influence $y$ to have large geometric sample values.
...
There is an important assumption hiding inside this linear bound: that the attacker cannot tell which of their forged tokens decoded to large geometric values. The bound holds precisely because, unable to decode, the attacker can’t cherry-pick—every forged request is a fresh blind sample, so pushing the count up takes linear effort. But the whole purpose of the system is to *publish counts*, and a published count is exactly the oracle the attacker otherwise lacks. Inject a token into an otherwise-sparse slice, watch whether the reported cardinality jumps, and you learn whether that token happened to decode high—without breaking any encryption. With that feedback an attacker can curate a collection of high-$k$ tokens, one per bucket, and a curated set is no longer linear: roughly $B$ requests can then push every bucket to level $k$, inflating the estimate to $\sim \frac{1}{\ln 2} B 2^k$, the same exponential leverage as the plaintext scheme. So without a mitigation the linear-resistance result holds only against an attacker who *cannot observe per-slice counts*. The mitigation is to **cap every reported estimate at the number of requests aggregated into the slice**: there cannot be more unique clients in a set of requests than there are requests, so the report is at most $R$ no matter what the sketch says. This bounds inflation unconditionally. An attacker who adds $F$ forged requests to a slice carrying $C_h$ honest clients across $R_h$ honest requests moves the report from $\approx C_h$ to at most $R_h + F$, an inflation of at most $F + (R_h - C_h)$; when the slice is near-deduplicated ($R_h \approx C_h$, the usual case) that is just $F$—coefficient exactly one, whether or not the attacker managed to decode anything. The curated exponential leverage above is clipped precisely because the flat-batch estimate $\sim \frac{1}{\ln 2} B 2^k$ is capped at the $B$ requests that produced it.

The decode oracle itself does not disappear, but the cap makes it expensive. To read rank $k$ the *uncapped* estimate must exceed the request count, so the attacker needs $\gtrsim \frac{1}{\ln 2} B 2^k$ requests before the reported value stops rising—exponential in the bit position $k$, and a full decode is infeasible. Extracting bit $k$ now costs exactly the inflation it would buy. The residual slack is honest duplication: a slice with $R_h \gg C_h$—a CI box pulling one package a million times—still permits inflation up to $R_h - C_h$ and lets the attacker piggyback ranks up to $\sim \log_2(R_h \ln 2 / B)$, so obvious request-floods should be damped where feasible.

This cap is *distinct* from the minimum-cardinality floor of the [Protocol Summary](06-protocol-summary.md), and the two should not be conflated: the floor protects the *anonymity* of small populations, while the cap protects *count integrity*.

It’s worth flagging a stronger mitigation that we have deliberately *not* adopted, since it’s a natural question. One could try to *bind each token to its request*—mixing something request-specific (a server-issued nonce, or the full request path) into the value the client sends—so that a single high-value forgery can’t be reused across many requests. The difficulty is that honest counting needs the opposite: a client’s value must be *stable* across its requests within a class, or we’d be counting requests instead of unique clients. A binding that preserved per-(client, class) stability while denying an attacker reuse of a curated value is not obviously achievable, and in any case the request-count cap already bounds inflation to the number of requests sent, without needing to prevent reuse at all. So we rely on the cap and leave token-request binding as an open question—to revisit only if some deployment needs to bound inflation below its raw request volume.
...
```math
\begin{aligned}
\gen{g, -1} = \gen{g} \sqcup -\gen{g} = J_N^+
\end{aligned}
```

The union is disjoint and fills $J_N^+$: the cyclic group $\gen{g}$ has order $\lambda(N) = (P-1)(Q-1)/4$—half the size of $J_N^+$, so index $2$—and $-1 \notin \gen{g}$, since $g^k = (k, k, k, k)$ can equal $-1 = (2, 0, 2^{m-1}, 0)$ only if $k \equiv 2 \pmod{4}$ while also $k \equiv 2^{m-1} \equiv 0 \pmod{4}$, which is impossible. So every product of known powers of $g$ and $-1$ has positive Jacobi symbol, and to forge, an attacker must instead reach $J_N^-$ with a known $C_{2^m}$-coordinate. The natural way to do that is a square root of $-1$: a primitive fourth root of unity $i$ has logarithm $(\text{odd}, 0, 2^{m-2}, 0)$ and $\Jacobi_N(i) = -1$, so it is a known-logarithm element of $J_N^-$—and given $i$, the forgery is immediate. Computing such an $i \bmod N$ is not known to be any easier than factoring $N$: we have a one-way chain, factor $N \Rightarrow$ compute $i \Rightarrow$ forge, with no reduction known in either reverse direction. (Indeed a *mixed* square root of $-1$, $\xi = \mathrm{CRT}(-1 \bmod P,\, 1 \bmod Q)$, has $\Jacobi_N(\xi) = 1$ and $\gcd(\xi - 1, N) = Q$, so it simply *is* the factorization; the server plainly cannot publish it.) But $i$ is *more* than a forgery strictly needs: forging requires only some $J_N^-$ element whose $C_{2^m}$-coordinate $c$ is known—not its full logarithm, and not $i$ in particular. A square root of $-1$ is thus a *sufficient* handle, and as far as we know a strictly harder one to obtain than the bare known-$c$ element a forgery calls for. The honest picture is therefore an upper bound: the gap between what a client can build and what a forgery needs is *at most* one square root of $-1$, with no matching lower bound.
...
All of this rests on $N$ being hard to factor—forging reduces to a square root of $-1 \bmod N$, no easier than factoring. Does the ring’s engineered structure weaken that? It does publish a little about the factors: $Q = 2^m q + 1$ fixes the low $m$ bits of $Q$, and $P = 4Bp + 1$ fixes $P \bmod 4B$, another $\approx \log_2(4B)$ low bits of $P$. Coppersmith-style lattice factoring can exploit known bits of a factor, but only once they reach about a quarter of $N$’s bits, $\approx L/4$ for a balanced semiprime. The leak is far below that—at $L = 1024$ the threshold is $256$ bits, against $m \le 128$ known bits of $Q$ (and $\approx 14$ of $P$ for $B \le 2^{12}$)—and known bits below the threshold give no factoring advantage at all, so the structure costs nothing: $N$ keeps the full strength of its size. The only constraint worth stating is $m \le L/4$, met with a wide margin for any secure $L$ and any sane $m$.

The size itself is a deliberate, and deliberately modest, choice: $L = 1024$, about an 80-bit security level—below what one would pick to protect confidential data. It holds up because of *what factoring actually buys an attacker here*. Factoring $N$ breaks **count integrity**, not anonymity: it lets an attacker decode tokens and forge ranks, but anonymity is information-theoretic given the $(b, k)$ leak and does not depend on $N$ at all—whoever factors $N$ learns only the $(b, k)$ the honest server already computes. So the asset behind the modulus is the accuracy of aggregate counts, and even that inflation is capped at the request count. An adversary spending an $\approx 2^{80}$ factoring effort to skew package-install statistics they could only inflate linearly anyway is a wildly poor trade—while a larger modulus would double the per-request token (a ring element is $\approx L$ bits), a permanent cost on every request. For a high-volume service like a package registry, $L = 1024$ is the right balance; a deployment with higher-stakes counts can simply raise it.

This is also why the client’s acceptance criteria bound $m$, $B$ and $L$ independently rather than coupling $m$ to $L$: modulus hardness is the *server’s own* integrity concern. A weak or undersized $N$ degrades the server’s counts, but a client that accepts one loses nothing—its anonymity holds regardless. So there is intentionally no minimum-size or $m$-vs-$L$ check on the client side; the size floor is guidance for whoever generates $N$, not a condition a client must enforce for its own protection.
...
In other words, $P-1$ is divisible by $2^m$ instead of just $4$ and $Q-1$ is divisible by $B$ when it shouldn’t be. With this alternative structure, $\Z_N^*$ has the following richer multiplicative structure:
...
Note that the certificate carries no semigenerator, and no shard base $f$. The shard base is *derived deterministically from $N$*—every client computes the same $f = \hash_{J^+}(N)^B$—rather than being chosen and published by the server. This closes an otherwise glaring fingerprinting channel: a *published* $f$ (or semigenerator) could be varied per client, and since a client’s requests all sit in the coset $x_0\gen{f}$, a distinct $f$ handed to each client would tag every request that client ever sends. Deriving $f$ from $N$ leaves the server nothing to vary—two clients that accept the same modulus necessarily use the same shard base. Raising the hash to the $B$ forces $f$'s $C_B$ component to vanish, and the server regenerates $N$ until the derived $f$ also shards the rank (which holds for all but a negligible fraction of moduli). Those two facts — a unit whose $C_B$ component is zero and whose rank coordinate is odd — are exactly the conditions for $f = g^{B'}$ with some semigenerator $g$ and some odd multiple $B'$ of $B$. So the server certifies $f$ as a valid shard base without ever computing $g$ or $B'$; the shard check needs the secret factor $Q$, so it is the server that runs it, at generation time. Every client then rederives the identical $f$ from $N$ and uses it blindly — it cannot repeat the shard check, but it need not, since a defective shard base would weaken only that client’s own sharding, never expose a fingerprint.
...
One last question a careful reader should ask: the certificate publishes $n$ square roots of hash-derived residues, and about half of them land in $J_N^-$—each challenge is a quadratic residue, its four roots share a single Jacobi symbol, so it is a coin flip per sample. Isn’t that handing out exactly the $J_N^-$ elements a forger needs? No, because a forger needs an element of $J_N^-$ *with a known logarithm*, and naming a $J_N^-$ element is already free ($\Jacobi_N(2) = -1$ for every ring of this shape). The published roots are just as opaque: their logarithms with respect to $g$ are unknown, so a root that happens to land in $J_N^-$ is no more useful than the number $2$. Nor can the roots be combined into a useful one. This is a Rabin signature over hash-derived messages, and its safety rests on three facts. First, the challenges are $\hash(N, \text{tag}, i)$, recomputable by anyone and *not adversary-chosen*—the classic Rabin break, asking for a second root of a value whose root you already know and reading a factor of $N$ off the difference, needs an injected challenge, which the hashing forbids. Second, each challenge is rooted at most once, and the root-finding is *deterministic*—the smallest root modulo each prime factor, combined by CRT—so a challenge value never yields two different roots, even across independent re-issuances of the certificate for the same $N$; two distinct roots of one value would factor $N$ outright. Third, since $\left(\prod r_i\right)^2 = \prod z_i$, the only leverage the roots offer is a multiplicative relation $\prod z_i^{e_i} = \zeta$ hitting some useful target $\zeta$; such relations *exist*—a few hundred random residues generate essentially the whole quadratic-residue subgroup—but *finding* one is a discrete-log computation mod $N$, as hard as the problem the whole scheme rests on. (This last point leans on the challenges being large, random residues; a hashing scheme that produced small or structured values could open a smoothness/index-calculus route and dissolve the argument.)
