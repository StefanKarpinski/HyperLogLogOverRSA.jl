...
The client randomly chooses and saves a persistent $x \in \Z_N^*$ value. For each new request, it randomly chooses $w \in (\Z_N^*)^{B2^{m-1}}$ and $t = 1 \bmod{B}$ and sends $y = wx^t$. It’s not too hard to intuitively see that the only meaningful pieces of information conveyed about $x$ are:
...
The $C_4$ component carries one bit beyond the parity, and it is worth pinning down why. Write $a = 2a_1 + a_0$ in binary digits and let $u_1$ be the bit of $c$ just above its lowest set bit. The parity $a_0$ is not free — the requirement $\Jacobi_N(x) = -1$ ties it to the parity of $c$ — but $a_1$ is the client’s own, and the bit that survives re-randomization is
...
Neither input survives alone. Multiplying an odd number by an odd $t$ flips its second bit by *$t$’s* second bit, since $(2t_1+1)(2x_1+1) = 2(t_1 \xor x_1) + 1 \bmod 4$. But the same $t$ scales $a$ and the odd part of $c$ alike, so $a_1$ and $u_1$ flip together and their xor comes out unchanged. (When $c$ is odd, $a$ is even and $ta = 2a_1 \bmod 4$ leaves $a_1$ alone outright, so $s = a_1$ there.) One bit of the $2$-part is therefore legible to the ring holder however $w$ and $t$ are drawn. Since nothing can erase it, the protocol uses it rather than fighting it: the bucket index is $b$ together with that bit, which is why the ring has $B$ buckets and not $\nfrac{B}{2}$. Declaring the bit is also what keeps it from being a one-bit fingerprint, as [the anonymity theorem](05-security-analysis.md#The-anonymity-theorem) demonstrates in a precise manner.

That same formula says where the geometric ladder has to stop. $u_1$ is a bit of $c$, and it exists only while $c$ has a bit above its leading one — that is, while $\tz(c) ≤ m-2$. On the top two rungs, where $c$ is $2^{m-1}$ or $0$, there is nothing for the xor to act on. So the reported geometric sample is capped at $m-1$, and at that capped level $s$ records *which* of the two rungs the token sits on instead. That is not a rounding-off: it makes the value space come out exactly, $m$ levels times one bit accounting for every distinguishable case, as the [security analysis](05-security-analysis.md#The-anonymity-theorem) works through.

The core HyperLogLog Over RSA construction is now in place. The server constructs a ring with the shape $N = PQ = (2Bp+1)(2^m q+1)$, keeping $P$, $Q$, $p$, $q$ secret and publishing $N$ together with a semigenerator $g$. A client generates a persistent secret $x \in \Z_N^*$ with $\Jacobi_N(x) = -1$ and, for each request, sends a freshly randomized token $y = wx^t$ where $t \equiv 1 \bmod{B}$ and $w$ is chosen from $(\Z_N^*)^{B2^{m-1}}$. The server discards any request where $\Jacobi_N(y) \neq -1$, since that indicates a misbehaving client. For the remaining requests, it uses the secret factors, $P$ and $Q$, to extract the geometric sample $k = \min(\tz(c),\, m-1)$ from the $C_{2^m}$ component and the bucket index $b$ from the $C_{2B}$ component — not quite as two independent reads, since the bucket’s low bit is the xor described above — yielding a decrypted HLL sample pair, $(b, k)$. Clients cannot steer or bias samples without knowing the factorization of $N$. The next section completes the protocol by making it work nicely with resource class sharding.
...
- The geometric sample: $\min(\tz(t(c + h)),\, m-1) \in \set{0, \dots, m-1}$

Here we can see that the real requirement on $h$ is that $(a + h, b + h, c + h)$ covers $\Z_4 \times \Z_{B/2} \times \Z_{2^m}$ fairly uniformly as $h$ takes on different values. Note that $h$ drives all three coordinates at once, so the coverage is of a cyclic subgroup rather than the full product — but the induced distribution on what is actually decoded is still the right one. The bucket index $b + h$ is uniform on $\Z_{B/2}$; the geometric sample has its intended geometric distribution because $c + h$ is uniform on $\Z_{2^m}$; and the extra bucket bit is balanced, because within each geometric level the bit above the leading bit of $c+h$ takes both values while $a+h$ is either fixed or moves in lockstep with it. To ensure this it’s sufficient to ensure that $h$ is sampled from a modulus, $M$ such that $\fmod(M, B2^{m-1})$ is tiny relative to $M \div B2^{m-1}$. We could use an exact multiple of $B2^{m-1}$, which makes the modulus zero. But that’s slightly inconvenient, since real world hashes have power-of-two outputs, so we’d have to do rejection sampling. Fortunately, if $M$ is sufficiently large, the bias (ratio) is negligible. For any modern hash function like SHA-256, this is the case. In our reference implementation, we actually use the output of HMAC-SHA-256 (keyed by the SHA-256 hash of $x_0$), truncated into a 128-bit integer value, which is still more than large enough to guarantee effectively uniform coverage.
...
- Compute $w = z^{B2^{m-1}}$ — one modular exponentiation
...
The bucket’s low bit is the exception. It is read off the $C_4$ coordinate against the leading odd factor of the $C_{2^m}$ coordinate, and $h$ moves both, so it varies from class to class along with the geometric sample. A semisharded client therefore occupies not one bucket but one of a fixed *pair* — $b_0$ and $b_0 + \nfrac{B}{2}$ — chosen per class. So semisharding pins a client to $2$ of the $B$ buckets across all of its classes rather than to a single one. Everything the property is used for is a statement about how *few* buckets a client’s classes can span, and two is a constant rather than a fresh draw each time, so the argument in the [security analysis](05-security-analysis.md#Request-bundles-and-cross-class-correlation) goes through with that constant doubled. No choice of $f$ avoids this: $s$ compares the $C_4$ coordinate against the leading bit of the $C_{2^m}$ coordinate, and the latter has to move from class to class for the geometric sample to shard at all. Semisharding thus keeps what HyperLogLog needs and drops nearly all of the per-class variation of the bucket.
