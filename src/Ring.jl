...
The `C_4` carries no HyperLogLog value — tokens are sent with an even exponent,
which collapses it to a parity the Jacobi condition already fixes. Its purpose is
arithmetic: it makes `P ≡ 5 mod 8` and hence `N ≡ 5 mod 8`, so `jacobi(-1, N) = 1`
and `-1` — the one element of the ring whose logarithms are public — stays out of
`J_N^-`, where it would otherwise be a ready-made forgery.
...
# Tokens arrive with an even exponent applied, so their C_2^m logarithm carries
# one more factor of two than the client's own: the walk below returns tz + 1,
# and starting the countdown at m-1 rather than m undoes that shift. It also caps
# the ladder at m-1, since the top two rungs both square to the identity.
